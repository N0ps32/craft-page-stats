...
class CraftStatsService extends Component
{
    ...
    /**
     * @var int start of request time
     */
    private $startRequest;

    public function startPerformanceTrace()
    {
        $this->startRequest = microtime(true) * 1000;
    }
    ...
    public function countView(string $url, string $query)
    {
        ...
        $pageHit->query = $query;
        $pageHit->url = $url;
        $pageHit->isBot = $this->isRequestMadeByBot($agent);
     ...
     * @return int
     */
    private function isRequestMadeByBot(string $agent): int
    {
        if ($agent === null) {
            return 0;
        }

        return preg_match('/bot|crawl|slurp|spider|mediapartners/i', $agent) === 1 ? 1 : 0;
    }

    /**
     * Returns if the website was requested by a mobile device
     *
     * @param string $agent
     * @return int
     */
    private function isMobileRequest(string $agent): int
    {
        if ($agent === null) {
            return 0;
        }
